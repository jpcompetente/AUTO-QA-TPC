...
export default function CameraReceiver({ sessionId: propSessionId }) {
      ...
      try {
        ...
        pc.onicecandidate = (ev) => {
          if (ev.candidate && wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            wsRef.current.send(JSON.stringify({ type: "ice", from: "receiver", data: ev.candidate }));
          }
        };
        ...
        ws.onmessage = async (evt) => {
          try {
            ...
            if (msg.type === "offer") {
              ...
              ws.send(JSON.stringify({ type: "answer", from: "receiver", data: answer }));
              setStatus("connected");
  ...
  return (
    <div className="camera-receiver">
      ...
      <video id="remote" ref={videoRef} autoPlay playsInline controls style={{ width: "100%", maxWidth: 800 }} />
      <p>Status: {status}</p>
    </div>
  );
}
